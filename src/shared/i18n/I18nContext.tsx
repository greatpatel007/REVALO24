...
import { DEFAULT_LOCALE, ensureDictionary, getDictionary, LOCALES } from "./dictionaries";
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const [dict, setDict] = useState(() => getDictionary(locale));

  useEffect(() => {
    let cancelled = false;
    setDict(getDictionary(locale));
    void ensureDictionary(locale).then((d) => {
  ...
  }, [dict, locale, location.pathname, location.search, navigate]);
