...
function Shell({ label, hint, error, required, id, errorId, children }: FieldShell & { id: string; errorId?: string; children: ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {error && <p id={errorId} role="alert" className="text-xs font-medium text-err-700">{error}</p>}
...
export function Select({ label, hint, error, required, className = "", children, ...rest }: FieldShell & SelectHTMLAttributes<HTMLSelectElement>) {
  ...
  return (
    <Shell label={label} hint={hint} error={error} required={required} id={id} errorId={errorId}>
      ...
      <span className="relative block">
        <select
          ...
          className={`${control} cursor-pointer appearance-none pr-9 ${error ? "border-err-600" : ""} ${className}`}
          {...rest}
        >
          {children}
        </select>
