...
/* ---- Lightbox: all photos + the video/floor-plan slots (IS24 exposé) ---- */
function MediaLightbox({ p, open, initialIdx, onClose }: { p: Property; open: boolean; initialIdx: number; onClose: () => void }) {
  const { t } = useI18n();
  const [tab, setTab] = useState<MediaTab>("photos");
  const [imgIdx, setImgIdx] = useState(initialIdx);

  return (
    <Modal open={open} onClose={onClose} title={`${t("detail.media")} — ${p.title}`} wide>
      <Tabs
        tabs={[{ id: "photos", label: t("detail.photos", { n: p.media.images.length }) }, { id: "video", label: t("detail.video") }, { id: "floorplan", label: t("detail.floorplan") }]}
        active={tab}
        onChange={setTab}
      />
      <div className="mt-4">
        ...
        {tab === "video" && (
          <div className="flex aspect-[16/9] items-center justify-center rounded-xl bg-navy text-slate-400">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm font-semibold">{t("detail.videoPh")}</p>
            </div>
          </div>
        )}
        {tab === "floorplan" && (
          <div className="flex aspect-[16/9] flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-400 bg-white text-muted">
            ...
            <p className="text-sm font-semibold">{t("detail.planPh")}</p>
          </div>
        )}
      </div>
    </Modal>
...
function Gallery({ p, onOpen }: { p: Property; onOpen: (idx: number) => void }) {
  const { t } = useI18n();
  ...
  return (
    <div className="relative">
      ...
      <div className="relative sm:hidden">
        ...
        >
          {imgs.map((src, i) => (
            <button key={src} type="button" onClick={() => onOpen(i)} aria-label={t("detail.openPhoto", { n: i + 1 })} className="w-full shrink-0 cursor-pointer snap-center">
              <img src={src} alt={`${p.title} — ${t("detail.photoOf", { n: i + 1 })}`} loading={i > 0 ? "lazy" : undefined} className="aspect-[4/3] w-full object-cover" />
            </button>
          ))}
        </div>
        <span className="absolute bottom-3 right-3 rounded-full bg-navy/75 px-2.5 py-1 text-xs font-bold text-white" aria-hidden>
      ...
      <div className="hidden aspect-[2/1] grid-cols-4 grid-rows-2 gap-2 overflow-hidden rounded-xl sm:grid">
        <button type="button" onClick={() => onOpen(0)} aria-label={t("detail.openPhoto", { n: 1 })} className="group col-span-2 row-span-2 cursor-pointer overflow-hidden">
        ...
        {imgs.slice(1, 5).map((src, i) => (
          <button key={src} type="button" onClick={() => onOpen(i + 1)} aria-label={t("detail.openPhoto", { n: i + 2 })} className="group cursor-pointer overflow-hidden">
      ...
      <button
        type="button"
        onClick={() => onOpen(0)}
        className="absolute bottom-3 right-3 hidden min-h-10 cursor-pointer items-center gap-2 rounded-lg bg-white px-4 text-sm font-bold text-navy shadow-elevation-md transition-colors hover:bg-blue-50 sm:flex"
      >
...
function FinancingCard({ p, locale }: { p: Property; locale: Locale }) {
  ...
  return (
    <div className="rounded-xl border border-slate-300 bg-white p-5 shadow-elevation-sm">
      <h2 className="mb-4 flex items-center gap-2 font-display text-base font-bold">
        <Calculator weight="duotone" className="size-5 text-blue-600" aria-hidden /> {t("detail.financing")}
      </h2>
    ...
    <div className="mx-auto max-w-shell px-4 pb-24 pt-6 sm:px-6 sm:pb-8 lg:pt-8">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="min-w-0">
          ...
          {lightbox !== null && (
            <MediaLightbox p={p} open initialIdx={lightbox} onClose={() => setLightbox(null)} />
          )}
