...
export function Header() {
  ...
  const [fontPair, setFontPair] = useState<FontPairId>(DEFAULT_FONT_PAIR_ID);
  const location = useLocation();

  useEffect(() => setMenuOpen(false), [location.pathname, location.search]);

  /* Hydrate lab selection after mount (boot script already applied CSS). */
  useEffect(() => {
    setFontPair(getStoredFontPairId());
  }, []);
    ...
    ? createPortal(
        <div className="fixed inset-0 z-[95]" role="presentation">
          ...
          >
            ...
            {/* Font-pair lab — evaluation only; not in primary nav */}
            <hr className="my-2 border-slate-300" />
            <p className="t-overline px-3 py-2 text-muted">{t("lab.overline")}</p>
            <div className="px-3 pb-2">
              <Select
                label={t("lab.typeface")}
                value={fontPair}
                onChange={(e) => setFontPair(applyFontPair(e.target.value))}
              >
                {FONT_PAIRS.map((p) => (
