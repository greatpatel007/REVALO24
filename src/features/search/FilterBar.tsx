...
import { BellRinging, CaretDown, Check, Faders, ListBullets, MapTrifold } from "@phosphor-icons/react";
...
/** Shared query string for List ↔ Map (drops list-only `page`). */
function viewSwitchQs(params: URLSearchParams) {
  const crossParams = new URLSearchParams(params);
  crossParams.delete("page");
  return crossParams.toString() ? `?${crossParams.toString()}` : "";
}

/** Inline List | Map control — same h-10 / rounded-lg as the Sort select beside it. */
export function ViewSwitch({ view }: { view: "list" | "map" }) {
  const { t, to } = useI18n();
  const { params } = useSearchFilters();
  const qs = viewSwitchQs(params);

  const seg = (active: boolean) =>
    `inline-flex h-full items-center gap-1.5 whitespace-nowrap rounded-md px-2.5 text-sm font-semibold transition-colors sm:px-3 ${
      active ? "bg-white text-navy shadow-elevation-sm" : "text-slate-600 hover:bg-white/70 hover:text-navy"
    }`;
  ...
  return (
    <div
      ...
      className="inline-flex h-10 items-stretch rounded-lg border border-border-strong bg-slate-100 p-0.5"
    >
      <Link to={to(`/properties${qs}`)} aria-current={view === "list" ? "page" : undefined} className={seg(view === "list")}>
        <ListBullets weight="bold" className="size-4 shrink-0" aria-hidden />
        {t("search.list")}
      </Link>
      <Link to={to(`/map${qs}`)} aria-current={view === "map" ? "page" : undefined} className={seg(view === "map")}>
        <MapTrifold weight="bold" className="size-4 shrink-0" aria-hidden />
        {t("nav.map")}
      </Link>
    </div>
  );
...
/** Map view: Zillow-style floating List | Map control (bottom-center).
    List view uses `ViewSwitch` next to Sort in the results toolbar instead. */
